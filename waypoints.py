...
def refine_franchised_bus_lines():
    with TemporaryDirectory(ignore_cleanup_errors=True) as work:
        ...
        base = cmd.split() if cmd else ["docker", "run", "--rm",
    ...
    for (rid, seq), tid in kept.items():
        ...
        if len(pts) < 2 or max_stop_distance(pts, numpy.array(coords, dtype=float)) > GATE_M:
...
try:
    ...
    # CSDI output above is already complete — publish it rather than fail the crawl
    logging.exception("pfaedle refinement failed; publishing CSDI lines only")
